...
if (class_exists("GFForms")) {
    ...
    class StickyList extends GFAddOn {
        ...
        function stickylist_shortcode( $atts ) {
            ...
            if($enable_list){
                ...
                if($show_entries_to === "creator"){
					...
					if($current_user_id!=136343 && $current_user_id!=65263 && $current_user_id!=64380 ){
                    ...
                    foreach ($entries as $entry) {
                        ...
                        if($enable_view || $enable_edit || $enable_delete || $enable_postlink || $enable_duplicate){
                            ...
                            $list_html .= "<td class='sticky-action'>";
                                ...
                                if($enable_view) {
                                    $list_html .= "
                                        <form action='$embedd_page?quote=appointment' method='post'>
        ...
        public function pre_entry_action($form) {
			...
			if(empty($_POST["mode"]) ){
				$_POST["mode"] = isset($_GET["mode"])?$_GET["mode"]:'';
			}
			
			if(empty($_POST["mode"]) ){
				$_POST["edit_id"] = isset($_GET["edit_id"])?$_GET["edit_id"]:'';
			}
            ...
            if( isset($_POST["mode"]) == "edit" || isset($_POST["mode"]) == "view" || isset($_POST["mode"]) == "duplicate") {
                ...
                if(isset($form_fields) && !is_wp_error($form_fields) && $form_fields["status"] == "active") {
